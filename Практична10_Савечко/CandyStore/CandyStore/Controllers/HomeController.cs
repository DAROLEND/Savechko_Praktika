...
namespace CandyStore.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public ActionResult SweetSearch(string name)
        {
            ...
            var results = db.Sweets.Where(s => s.Name.Contains(name)).ToList();

            if (results.Count <= 0)
            {
                return PartialView("_SweetSearch", new List<Sweet>()); 
            }

            return PartialView("_SweetSearch", results);
        }
